...
use rsext4::{
    BLOCK_SIZE,
    dir::{get_inode_with_num, mkdir_with_ino},
    disknode::{Ext4Extent, Ext4Inode},
    entries::{Ext4DirEntry2, classic_dir},
    extents_tree::ExtentTree,
    file::{mkfile_with_ino, truncate_with_ino, write_file_with_ino},
    link,
...
fn read_symlink_target(
    ...
    if inode.have_extend_header_and_use_extend() {
        let blocks = resolve_inode_block_allextend(fs, dev, inode).map_err(into_vfs_err)?;
        for phys in blocks {
            let cached = fs
                .datablock_cache
        ...
        for lbn in 0..total_blocks {
            let phys = match resolve_inode_block(dev, inode, lbn as u32).map_err(into_vfs_err)? {
                Some(b) => b,
                None => break,
            };
            let cached = fs
...
impl Rsext4Node {
    ...
    fn lookup_locked(&self, name: &str) -> VfsResult<DirEntry> {
        ...
        let inner = &mut *inner;
        let (fs, dev) = (&mut inner.fs, &mut inner.dev);
        let res = get_inode_with_num(fs, dev, &child_path)
            .map_err(into_vfs_err)?
...
impl NodeOps for Rsext4Node {
    ...
    fn metadata(&self) -> VfsResult<Metadata> {
        ...
        Ok(Metadata {
            ...
            blocks: ((inode.blocks_count() as u64) * 512) / (BLOCK_SIZE as u64),
...
impl FileNodeOps for Rsext4Node {
    fn read_at(&self, buf: &mut [u8], offset: u64) -> VfsResult<usize> {
        ...
        for lbn in start_lbn..=end_lbn {
            let phys = match resolve_inode_block(dev, &mut inode_for_map, lbn as u32)
                .map_err(into_vfs_err)?
            {
                Some(b) => b as u64,
                None => {
    ...
    fn write_at(&self, buf: &[u8], offset: u64) -> VfsResult<usize> {
        ...
        write_file_with_ino(dev, fs, self.ino, offset, buf).map_err(into_vfs_err)?;
    ...
    fn set_len(&self, len: u64) -> VfsResult<()> {
        ...
        let inner = &mut *inner;
        let (fs, dev) = (&mut inner.fs, &mut inner.dev);
        truncate_with_ino(dev, fs, self.ino, len).map_err(into_vfs_err)
    }
    ...
    fn set_symlink(&self, target: &str) -> VfsResult<()> {
        ...
        if inode.blocks_count() != 0 {
            if inode.have_extend_header_and_use_extend() {
                let used_blocks =
                    resolve_inode_block_allextend(fs, dev, &mut inode).map_err(into_vfs_err)?;
        ...
        if target_len == 0 {
            ...
            if fs.superblock.has_extents() {
                ...
                for lbn in 0..(total_blocks as u32) {
                    ...
                    let end = core::cmp::min(start + BLOCK_SIZE, target_bytes.len());
                ...
                let data_sectors = total_blocks.saturating_mul((BLOCK_SIZE / 512) as u64);
                ...
                let iblocks_used = total_blocks.saturating_mul((BLOCK_SIZE / 512) as u64);
...
impl DirNodeOps for Rsext4Node {
    fn read_dir(&self, offset: u64, sink: &mut dyn DirEntrySink) -> VfsResult<usize> {
        ...
        let inner = &mut *inner;
        let (fs, dev) = (&mut inner.fs, &mut inner.dev);
        let dir_inode = fs.get_inode_by_num(dev, self.ino).map_err(into_vfs_err)?;
        if !dir_inode.is_dir() {
        ...
        if dir_inode.have_extend_header_and_use_extend() {
            let blocks = resolve_inode_block_allextend(fs, dev, &mut dir_inode.clone())
                .map_err(into_vfs_err)?;
            for phys in blocks {
            ...
            for lbn in 0..total_blocks {
                let phys = match resolve_inode_block(dev, &mut dir_inode.clone(), lbn as u32)
                    .map_err(into_vfs_err)?
                {
                    Some(b) => b,
                    None => continue,
    ...
    fn create(
        ...
        if get_inode_with_num(fs, dev, &child_path)
            .map_err(into_vfs_err)?
            .is_some()
        {
        ...
        match node_type {
            NodeType::Directory => {
                let Some((ino, _inode)) = mkdir_with_ino(dev, fs, &child_path) else {
            ...
            NodeType::RegularFile => {
                let Some((ino, _inode)) = mkfile_with_ino(dev, fs, &child_path, None, None) else {
            ...
            NodeType::Symlink => {
                ...
                let Some((ino, _inode)) = mkfile_with_ino(
                    dev,
                    fs,
                    &child_path,
                    None,
                    Some(Ext4DirEntry2::EXT4_FT_SYMLINK),
                ) else {
    ...
    fn link(&self, _name: &str, _node: &DirEntry) -> VfsResult<DirEntry> {
        ...
        if get_inode_with_num(fs, dev, &link_path)
            .map_err(into_vfs_err)?
            .is_some()
        {
            return Err(VfsError::AlreadyExists);
        ...
        link(fs, dev, &link_path, &linked_path);
