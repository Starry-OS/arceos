...
impl SegmentHeader {
    /// Returns the payload length (including padding).
    pub fn padded_payload_len(&self) -> AxResult<usize> {
        // Validate `self.len`.
        let payload_len = (self.len as usize)
            .checked_sub(size_of::<Self>())
            .ok_or_else(|| ax_err_type!(InvalidInput, "the message length is too small"))?;

        Ok(align_up(payload_len, NLMSG_ALIGN))
    }
}
